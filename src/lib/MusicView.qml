/**
      ...
      Rectangle {
       ...
       property int imageSize: (vertical)? Math.round( musicCarousel.height / imageAmount ) : Math.round( musicCarousel.width / imageAmount );
       ...
       property XmlListModel testList: XmlListModel {
           id: coverList
           source: xmlPath
           query: "/transferfiles/albums/album"

           XmlRole { name: "id"; query: "@id/string()" }
           XmlRole { name: "artist"; query: "artist/string()" }
           XmlRole { name: "album"; query: "title/string()" }
       ...
       property bool vertical: false
	    ...
	    Component.onCompleted: {
                if( musicCarousel.vertical ) width = imageSize;
	        else height = imageSize;
	        pathElement.path = (vertical)? verPath : horPath;
                goTo(1); goTo(0);
	    }
            onHeightChanged: if( !vertical && musicCarousel.height < imageSize ) height = imageSize;
            onWidthChanged: if( vertical && musicCarousel.width < imageSize ) width = imageSize;
	    ...
	    Component {
	        ...
	        Item {
	            ...
	            Component.onCompleted: {
                        if( musicCarousel.vertical ) anchors.horizontalCenter = pathElement.horizontalCenter;
	                else anchors.verticalCenter = pathElement.verticalCenter;
	            }

                    width: musicCarousel.imageSize
                    height: musicCarousel.imageSize
	            opacity: PathView.elementOpacity
                    Image {id: kuva; anchors.centerIn: parent; width: musicCarousel.imageSize; height: musicCarousel.imageSize; source: "../img/testdata/covers/"+cover; smooth: true; }
                    ...
                    Text {
                        ...
                        font.pointSize: theme.musicTransferArtistTextFontPointSize;
                        anchors.top: kuva.bottom;
                        text: artist;
                        width: kuva.width;
                        wrapMode: Text.Wrap;
                        color: theme.musicTransferArtistTextColor;
                    ...
                    Text {
                        id: albumname;
                        font.family: theme.generalFontFamily;
                        font.pointSize: theme.musicTranferAlbumTextFontPointSize;
                        text: album;
                        anchors.top: artistname.bottom;
                        width: kuva.width;
                        wrapMode: Text.Wrap;
                        color: theme.musicTransferAlbumTextColor;
                        horizontalAlignment: Text.AlignHCenter;
	            ...
	            z: PathView.elementZ
	            transform: Rotation {
                        origin.x: Math.round(musicCarousel.imageSize/2); origin.y: Math.round(musicCarousel.imageSize/2)
                        axis.x: (musicCarousel.vertical)?1:0; axis.y: (musicCarousel.vertical)?0:1; axis.z: 0     // rotate around y-axis
	    ...
	    PathView {
	        ...
	        model: coverList
	        delegate: delegate
                pathItemCount: musicCarousel.imageAmount
